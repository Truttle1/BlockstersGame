...
void MonsterUI::saveMonster()
{
	...
	if(Species::monsterSpecies[Species::monsterSpecies.size()-1].behaviors[Behaviors::SENTIENCE])
	{
		GameWindow::setCivilizedMode(true);
		GameWindow::transitionToCivilized();
	}
...
void MonsterUI::initBehaviors()
{
	...
	addTexts.clear();
	addTexts.push_back("Once you evolve a monster with this");
	addTexts.push_back("behavior, the game switches to");
	addTexts.push_back("Sentience Mode. This monster will then");
	addTexts.push_back("become the ONLY playable monster for the");
	addTexts.push_back("rest of the game.");
	addTexts.push_back("");
	addTexts.push_back("If you do NOT want this");
	addTexts.push_back("to happen, remove this behavior.");
	addTexts.push_back("If you actually like progress,");
	addTexts.push_back("evolve this behavior.");
	addTexts.push_back("");
	addTexts.push_back("Trust me, this behavior is 100% worth");
	addTexts.push_back("the 16 Evolve Points.");
	civilization.setWarning("HEY YOU! READ THIS!!!",addTexts);
	...
	if(getClicking(behaveX,behaveY,64,32))
	{
		for(unsigned i=0; i<behaviors.size(); i++)
		{
			if(behaviors[i].getStatus() == 1)
			{
				...
				std::vector<std::string> behaveMessage = behaviors[i].getMessage();
				std::string message = "";
				if(behaviors[i].getHeader() != "DEFAULT")
				{
					for(unsigned j = 0; j < behaveMessage.size(); j++)
					{
						message += behaveMessage[j];
						message += "/";
						cout << j << " YEE" << endl;
					}
					GameWindow::getMessageBox()->enable(behaviors[i].getHeader() + "/" + message);
				}
			}
